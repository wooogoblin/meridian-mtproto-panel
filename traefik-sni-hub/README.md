...
```bash
REPO="https://raw.githubusercontent.com/wooogoblin/vpntools/panel/traefik-sni-hub"
...
**Что умеет панель:**
- Добавлять и удалять пользователей (каждый получает уникальный MTProto-секрет) — до 16 пользователей
- Включать / отключать пользователей без перезапуска контейнера (SIGHUP hot-reload)
- Показывать количество активных соединений
- Генерировать `tg://` ссылки, QR-коды и секреты для каждого пользователя
...
- `install-panel.sh` — `python3`, `python3-pip`, `python3-venv`, `curl`, `dnsutils`, `openssl`, `xxd` (~10 МБ) + изолированный venv в `/opt/meridian/.venv` с `bcrypt` и `toml`. Сначала ставит из wheel — без компилятора. Если wheel недоступен (экзотическая архитектура), доставит `build-essential` + `python3-dev` и соберёт из исходников.
